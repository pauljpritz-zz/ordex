...
contract('1st OrDex test', async (accounts) => {
  ...
  function makeTransaction() {
    return {
      ...
      amounts: [4 * multiplier, 2 * multiplier],
      nonces: [1234, 5678],
      expiries: [1000, 1000]
    };
  }
  ...
  it("OrDex Swap", async () => {
    ...
    assert.equal(dogBalance.valueOf(), 16 * multiplier);

    const catBalance = await catContract.balanceOf.call(accounts[1], { from:accounts[1] });
    assert.equal(catBalance.valueOf(), 8 * multiplier);
  });
  ...
  it('should fail when called twice with same nonce', async () => {
    ...
    assert.equal(dogAllowance, 4 * multiplier);
    const catAllowance = await catContract.allowance.call(accounts[1], OrDex.address, { from: accounts[1] });
    assert.equal(catAllowance, 2 * multiplier);

    const dogBalance = await dogContract.balanceOf.call(accounts[0], { from: accounts[0] });
    assert.isAtLeast(parseInt(dogBalance, 10), 4 * multiplier);

    const catBalance = await catContract.balanceOf.call(accounts[1], { from: accounts[1] });
    assert.isAtLeast(parseInt(catBalance, 10), 2 * multiplier);
