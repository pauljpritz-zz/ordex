...
contract OrDex {
  ...
  function swap(address[2] wallet, IERC20[2] token, uint256[2] amount, uint256[2] nonce, uint256[2] expiry, bytes32[2] r, bytes32[2] s, uint8[2] v) public {

    for (uint8 i = 0; i < 2; i++) {
      bytes32 authorization = keccak256(abi.encodePacked(
        'OrDex',
        nonce[i],
        expiry[i],
  			address(token[i]),
        ...
        authorization = keccak256(abi.encodePacked(
